...
local VehicleGroup = VehicleTab:AddLeftGroupbox("飞车功能")
...
VehicleGroup:AddToggle("FlyCarToggle", {
    Text = "开始飞车",
    Default = false,
    Callback = function(v)
        if v then
            ...
            if not getgenv().FlyCarControllerRunning then
                ...
                getgenv().FlyCarController = task.spawn(function()
                    while getgenv().FlyCarControllerRunning do
                        ...
                        if hrp2 then
                            hrp2.Anchored = false
                            local oldBV = hrp2:FindFirstChildOfClass("BodyVelocity")
                            if oldBV then pcall(function() oldBV:Destroy() end) end
                            local oldBG = hrp2:FindFirstChildOfClass("BodyGyro")
                            if oldBG then pcall(function() oldBG:Destroy() end) end
                            local bv = Instance.new("BodyVelocity")
                            bv.Parent = hrp2
                            bv.MaxForce = Vector3.new(math.huge, math.huge, math.huge)
                            bv.Velocity = workspace.CurrentCamera.CFrame.LookVector * (getgenv().FlyCarSpeed or 50)
            ...
            getgenv().FlyCarControllerRunning = false
            if getgenv().FlyCarController then
