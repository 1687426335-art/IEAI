-- ==================== 卡密验证系统 ====================
-- 包含100个预生成卡密: 天卡25个, 周卡25个, 月卡25个, 永久卡25个
-- 验证成功自动执行脚本
...
inputBox.PlaceholderText = "输入卡密 (例: WDF-K4M8R2N7P9-DAY)"
...
confirmBtn.MouseButton1Click:Connect(function()
    ...
    if keyData.days ~= -1 then
        local createdAt = keyData.createdAt or 0
        if createdAt > 0 then
            local elapsed = os.time() - createdAt
            if elapsed > keyData.days * 86400 then
                onFail("卡密已过期")
                return
            end
        end
...
-- ==================== 控制台打印卡密列表（方便查看） ====================
print("===== 已加载 " .. table.getn(ALL_KEYS) .. " 个卡密 =====")
local count = { DAY = 0, WEEK = 0, MONTH = 0, FOREVER = 0 }
for key, data in pairs(ALL_KEYS) do
    count[data.type:upper()] = (count[data.type:upper()] or 0) + 1
end
print("天卡: " .. count.DAY .. " 个")
print("周卡: " .. count.WEEK .. " 个")
print("月卡: " .. count.MONTH .. " 个")
print("永久卡: " .. count.FOREVER .. " 个")
