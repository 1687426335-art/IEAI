...
for i=1,#username do local ci=(i-1)%#gradientColors+1 coloredUsername=coloredUsername..'<font color="'..gradientColors[ci]..'">'..username:sub(i,i)..'</font>' end
local version="v3.7"
local coloredVersion=""
for i=1,#version do local ci=(i-1)%#gradientColors+1 coloredVersion=coloredVersion..'<font color="'..gradientColors[ci]..'">'..version:sub(i,i)..'</font>' end
WindUI:Popup({Title='<font color="'..gradientColors[1]..'">wdf</font><font color="'..gradientColors[5]..'">ex</font>',IconThemed=true,Content="尊敬的用户 "..coloredUsername.." \n您使用的 <font color='"..gradientColors[1].."'>wdf</font><font color='"..gradientColors[5].."'>ex</font> 当前版本型号是: "..coloredVersion.."\n脚本已就绪！",Buttons={{Title="取消",Callback=function() end,Variant="Secondary"},{Title="执行",Icon="arrow-right",Callback=function() Confirmed=true createUI() end,Variant="Primary"}})
function createUI()
...
local connections={}
local waypointTpEnabled=false
local waypointCooldown=0
local oldNamecallForWaypoint
oldNamecallForWaypoint=hookmetamethod(game,"__namecall",function(self,...)
local method=getnamecallmethod()
local args={...}
if waypointTpEnabled and (method=="InvokeServer" or method=="FireServer") and self.Name=="PlayerFunc" then
if args[1]=="getNavigationPoint" then
local x=tonumber(args[2])
local z=tonumber(args[3])
if x and z and tick()-waypointCooldown>0.5 then
waypointCooldown=tick()
task.spawn(function()
local char=player.Character
if not char then return end
local hrp=char:FindFirstChild("HumanoidRootPart")
if not hrp then return end
local ro=Vector3.new(x,5000,z)
local rd=Vector3.new(0,-10000,0)
local rp=RaycastParams.new()
rp.FilterDescendantsInstances={char}
rp.FilterType=Enum.RaycastFilterType.Exclude
local result=workspace:Raycast(ro,rd,rp)
if result then
hrp.CFrame=CFrame.new(result.Position+Vector3.new(0,3,0))
WindUI:Notify({Title="地图传送",Content="已传送至标点",Duration=2})
else
WindUI:Notify({Title="地图传送",Content="无法找到地面位置",Duration=2})
end
end)
end
end
end
return oldNamecallForWaypoint(self,...)
end)
...
label.Parent=frame
local ti=TweenService:Create(frame,TweenInfo.new(0.3,Enum.EasingStyle.Back,Enum.EasingDirection.Out),{Position=UDim2.new(1,-230,1,-80)})
ti:Play()
task.wait(2)
local to=TweenService:Create(frame,TweenInfo.new(0.3,Enum.EasingStyle.Quad,Enum.EasingDirection.In),{Position=UDim2.new(1,0,1,-80)})
to:Play()
to.Completed:Connect(function() sg:Destroy() end)
end
local Window=WindUI:CreateWindow({Title='wdfex-Hub',Icon="heart",IconThemed=true,Author=version,Folder="CloudHub",Size=UDim2.fromOffset(580,440),Transparent=true,Theme="Dark",HideSearchBar=false,ScrollBarEnabled=true,Resizable=true,Background="https://raw.githubusercontent.com/XxwanhexxX/UN/main/preview_png.png",BackgroundImageTransparency=0.5,User={Enabled=true,Callback=function()
local userId=player.UserId
local tt=Enum.ThumbnailType.HeadShot
local ts=Enum.ThumbnailSize.Size420x420
local ok,th=pcall(function() return Players:GetUserThumbnailAsync(userId,tt,ts) end)
local av=ok and th or "rbxassetid://0"
local name=player.Name
local days=player.AccountAge
local regTime=days.." 天前"
local pg=Instance.new("ScreenGui")
pg.Name="UserInfoPopup"
pg.ResetOnSpawn=false
pg.ZIndexBehavior=Enum.ZIndexBehavior.Sibling
pg.Parent=player:WaitForChild("PlayerGui")
local frame=Instance.new("Frame")
...
avi.Size=UDim2.new(0,60,0,60)
avi.Position=UDim2.new(0.5,-30,0,15)
avi.BackgroundColor3=Color3.fromRGB(50,50,70)
avi.Image=av
...
nl.Size=UDim2.new(1,-20,0,30)
nl.Position=UDim2.new(0,10,0,85)
...
tl.Position=UDim2.new(0,10,0,120)
...
cb.Position=UDim2.new(0.5,-30,1,-40)
...
bc.Parent=cb
cb.MouseButton1Click:Connect(function() pg:Destroy() end)
end,Anonymous=false},SideBarWidth=250,Search={Enabled=true,Placeholder="搜索...",Callback=function(s) end},SidePanel={Enabled=true,Content={{Type="Button",Text="wdfex-Hub",Style="Subtle",Size=UDim2.new(1,-20,0,30),Callback=function() end}}}})
...
if mainGui then
local mf=mainGui:FindFirstChildOfClass("Frame")
if mf then
local s1=Instance.new("UIStroke")
s1.Thickness=3
s1.Color=Color3.fromHSV(0,1,1)
s1.Transparency=0.5
s1.ApplyStrokeMode=Enum.ApplyStrokeMode.Border
s1.Parent=mf
local s2=Instance.new("UIStroke")
s2.Thickness=5
s2.Color=Color3.fromHSV(0.5,1,1)
s2.Transparency=0.3
s2.ApplyStrokeMode=Enum.ApplyStrokeMode.Border
s2.Parent=mf
local h1=0
local h2=0.5
local cc=RunService.Heartbeat:Connect(function()
h1=(h1+0.01)%1
h2=(h2-0.01)%1
s1.Color=Color3.fromHSV(h1,1,1)
s2.Color=Color3.fromHSV(h2,1,1)
end)
...
local t=TweenService:Create(banner,TweenInfo.new(16,Enum.EasingStyle.Linear,Enum.EasingDirection.Out),{Position=UDim2.new(1,10,0,2)})
t:Play()
t.Completed:Connect(function()
banner.Position=UDim2.new(0,-tw,0,2)
startAnim()
end)
end
task.wait(0.5)
...
local bm=stuff:FindFirstChild("Black Market")
local targetItem=nil
local displayName=""
if bm then
for _,itemInfo in ipairs(toolItems) do
if itemInfo.name==selectedToolItem then
local slot=bm:FindFirstChild(itemInfo.id)
...
if targetItem then
local ok=pcall(function() event:InvokeServer("purchase",{isRestaurant=false,item=targetItem}) end)
if ok then showBuySuccess(displayName) end
else
...
if targetItem then
local ok=pcall(function() event:InvokeServer("purchase",{isRestaurant=true,quantity=1,item=targetItem}) end)
if ok then showBuySuccess(displayName) end
else
...
if team then
local tn=team.Name
if tn:find("Police") or tn:find("警察") or tn:find("Cop") then
...
root.Velocity=forceVec.Unit*(policeDodgeForce*5)
end
end)
end
...
InteractTab:Toggle({Title="自动捡钱",Value=false,Callback=function(value) autoPickupCash=value end})
task.spawn(function()
while not isDestroyed do
if autoPickupCash then
for _,obj in ipairs(workspace:GetDescendants()) do
if obj.Name=="CashDrop" and (obj:IsA("BasePart") or obj:IsA("Model")) then
pcall(function()
ReplicatedStorage.Remote.PlayerFunc:InvokeServer("cashDrop",obj)
end)
break
end
end
...
if n:find("stamina") or n:find("energy") then obj.Value=9999999 end
end
end
local char=player.Character
if char then
...
local cf=Workspace:FindFirstChild("Characters") and Workspace.Characters:FindFirstChild(player.Name)
if cf then
for _,gun in ipairs(cf:GetChildren()) do
...
local isPolice=tn:find("警察") or tn:find("Police") or tn:find("Cop")
local isCivilian=false
if p.Team then local t=p.Team.Name isCivilian=t:find("平民") or t:find("Citizen") or t:find("圣奥里公民") else isCivilian=true end
...
local deliveryNames={}
for _,data in ipairs(DELIVERY_TELEPORTS) do table.insert(deliveryNames,data.n) end
...
D:Divider({Text="地图标点传送"})
D:Toggle({Title="点击地图标点自动传送",Value=false,Callback=function(value) waypointTpEnabled=value WindUI:Notify({Title="地图传送",Content=value and "已开启，点地图标点试试！" or "已关闭",Duration=3}) end})
...
local SONG_LIST={{name="半壶纱",id="140168001118478"},{name="对你有感觉",id="113476583412576"},{name="失眠",id="124928120639248"},{name="中国人能飞",id="79254667830418"},{name="忘情牛肉面",id="72954292508946"},{name="无需多言",id="114940361500053"},{name="出山",id="108542841138539"},{name="来个好梗绷一绷",id="120070812635771"},{name="孤独患者",id="88257174439605"},{name="幻昼",id="103093530102792"},{name="海屿你",id="76421239273915"},{name="于是",id="132959953803661"},{name="罗生门",id="79952466129206"},{name="茫",id="72194943092340"},{name="忘不掉的你",id="91111816286323"},{name="DearD",id="139047831212058"},{name="戒烟",id="137671588958836"},{name="IQOO进行曲",id="109693244185458"},{name="祖国人进行曲",id="86555185586884"},{name="十年咕嘎无人知",id="78729794283728"},{name="unhappy",id="88523902860927"}}
...
elseif playMode=="顺序播放" then local ni=currentPlayIndex+1 if ni>#SONG_LIST then ni=1 end PlaySongByIndex(ni)
elseif playMode=="随机播放" then local ri=math.random(1,#SONG_LIST) while ri==currentPlayIndex and #SONG_LIST>1 do ri=math.random(1,#SONG_LIST) end PlaySongByIndex(ri) end
end)
end)
end
