...
function createUI()
    ...
    local function createDynamicIsland()
        ...
        glow2.Thickness = 6
        glow2.Color = Color3.fromRGB(100, 150, 255)
        ...
        local function breatheLoop()
            while gui and gui.Parent do
                local t1 = TweenService:Create(glow1, TweenInfo.new(1.5, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut), { Transparency = 0.5 })
                local t2 = TweenService:Create(glow1, TweenInfo.new(1.5, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut), { Transparency = 0.85 })
                local t3 = TweenService:Create(glow2, TweenInfo.new(2, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut), { Transparency = 0.7 })
                local t4 = TweenService:Create(glow2, TweenInfo.new(2, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut), { Transparency = 0.92 })
                local dot1 = TweenService:Create(dot, TweenInfo.new(0.8, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut), { Size = UDim2.new(0, 12, 0, 12), Position = UDim2.new(0, 9, 0.5, -6) })
                local dot2 = TweenService:Create(dot, TweenInfo.new(0.8, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut), { Size = UDim2.new(0, 10, 0, 10), Position = UDim2.new(0, 10, 0.5, -5) })

                t1:Play() t3:Play() dot1:Play()
                task.wait(0.8)
                t2:Play() t4:Play() dot2:Play()
                task.wait(1.2)
            end
        end
        ...
        local slideIn = TweenService:Create(container, TweenInfo.new(0.6, Enum.EasingStyle.Back, Enum.EasingDirection.Out), { Position = endPos })
        slideIn:Play()

        return gui, container
    end

    local islandGui, islandContainer = createDynamicIsland()
    ...
    -- 顺序：玩家修改 → 飞天与加速 → 互动 → 枪械功能 → 杀戮光环 → 传送点 → 透视 → 自动躲警察
                            ...
                            if targetPart then
                                ...
                                if dist < policeDodgeDistance then
                                    if policeDodgeWallCheck then
                                        local visible = isVisible(myPos, targetPart.Position, {char, pChar})
    ...
    InteractTab:Slider({ Title = "按住时间", Step = 0.1, Value = { Min = 0, Max = 10, Default = 0 }, Callback = function(value) Settings.HoldTime = value; if interactEnabled then ScanPrompts() end end })
    InteractTab:Slider({ Title = "触发距离", Step = 1, Value = { Min = 5, Max = 150, Default = 25 }, Callback = function(value) Settings.Distance = value; if interactEnabled then ScanPrompts() end end })
    ...
    local StaminaEvent
    pcall(function() StaminaEvent = ReplicatedStorage:WaitForChild("Remote", 5):WaitForChild("PlayerEvent", 5) end)
    if StaminaEvent then
        ...
        oldNamecall = hookmetamethod(game, "__namecall", function(self, ...)
            ...
            if self == StaminaEvent and method == "FireServer" then
                ...
                if args[1] == "takeDamage" and godOn then return end
            end
    ...
    task.spawn(function()
        while not isDestroyed do
            if staminaOn and StaminaEvent then pcall(function() StaminaEvent:FireServer("setStaminaOrFood", "stamina", 100) end) end
            task.wait(0.3)
        end
    end)
    ...
    local function zzRestore()
        if zzAffected and zzAffected.Parent then
            pcall(function() zzAffected.Size = Vector3.new(2, 1, 1) zzAffected.Transparency = 0 end)
        end
        zzAffected = nil
    end
        ...
        while not isDestroyed do
            if zzEnabled then
                ...
                if best ~= zzAffected then
                    ...
                    if best then
                        zzAffected = best
                        pcall(function() best.Size = Vector3.new(500, 500, 500) best.Transparency = 1 best.CanCollide = false end)
                    end
                end
            else
                zzRestore()
    ...
    local function DestroyTargetDisplay()
        if targetDisplayGui then targetDisplayGui:Destroy() targetDisplayGui = nil targetDisplayLabel = nil end
    end
            ...
            if targetHead then
                ...
                if myHead then
                    ...
                    pcall(function()
                        ...
                        if handleShots then handleShots:FireServer("2", "Shoot") end
                    end)
                end
            end
        end
    ...
    player.CharacterAdded:Connect(function()
        if kaEnabled then task.wait(0.05) performAttack() end
    end)

    local function onToolAdded(tool) if kaEnabled then performAttack() end end
    local function setupToolListener(char)
        if char then
            char.DescendantAdded:Connect(function(desc) if desc:IsA("Tool") then onToolAdded(desc) end end)
        end
    end
    ...
    C:Divider({ Text = "显示设置" })
    C:Toggle({ Title = "显示攻击目标", Value = true, Callback = function(value)
        showTarget = value
        if value then if kaEnabled then CreateTargetDisplay() UpdateTargetDisplay() end
        else DestroyTargetDisplay() end
    end })
    ...
    local ESP_SHOW_TEAM = false
    local ESP_SHOW_HEALTH = false
    ...
    local function GetTeam(p)
        if p.Team then
            ...
            local teamMap = {
                ...
                ["Road"] = "道路服务", ["Civilian"] = "平民", ["Citizen"] = "平民",
                ["Delivery"] = "转运", ["Farmer"] = "农民"
            }
    ...
    local function BuildESP(p)
        ...
        if ESP_LIST[p.UserId] then
            if ESP_LIST[p.UserId].Billboard then ESP_LIST[p.UserId].Billboard.Enabled = true end
            return
        end
    ...
    E:Toggle({ Title = "显示队伍", Value = false, Callback = function(value) ESP_SHOW_TEAM = value; if ESP_ENABLED then RefreshESP() end end })
    ...
    MusicGroup:Dropdown({ Title = "选择歌曲", Values = songNames, Value = songNames[1], Callback = function(value)
        for i, song in ipairs(SONG_LIST) do
            if song.name == value then selectedSong = song currentPlayIndex = i break end
        end
    ...
    MusicGroup:Slider({ Title = "音量", Step = 0.1, Value = { Min = 0, Max = 7, Default = 1 }, Callback = function(value)
        if musicSound then musicSound.Volume = math.min(value, 1) end
    end })

    MusicGroup:Divider()
    MusicGroup:Paragraph({ Title = "播放模式", Desc = "选择音乐的播放方式" })

    MusicGroup:Dropdown({ Title = "播放模式", Values = { "顺序播放", "循环播放", "随机播放" }, Value = "顺序播放", Callback = function(value)
        playMode = value
