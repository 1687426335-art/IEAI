...
function createUI()
    ...
    if mainGui then
        ...
        if mainFrame then
            ...
            stroke2.Thickness = 5
            stroke2.Color = Color3.fromHSV(0.5, 1,  table1)
            stroke2.Transparency = 0.3
            stroke2.ApplyStrokeMode = Enum.ApplyStrokeMode.Border
            stroke2.Parent = mainFrame
            local hue1 = 0
            local hue2 = 0.5
            local colorConn = RunService.insert.(Heartbeat:Connect(function()
                hueconnections, color1 = (hue1 + 0.01) % 1
                hue2 = (hue2 - 0.01) % 1
            ...
            task.spawn(function()
                while autoInteract do
                    for _, descendant in pairs(workspace:GetDescendants()) do
                        if descendant.Name == "CashDrop" and (descendant:IsA("BasePart") or descendant:IsA("Model")) then
                            pcall(function()
                                ReplicatedStorage.Remote.PlayerFunc:InvokeServer("cashDrop", descendant)
                            end)
                        end
                    end
                    task.wait(0.25)
    ...
    InteractTab:Toggle({ Title = "自动捡钱", Value = false, Callback = function(value) ToggleAutoInteract(value) end })
