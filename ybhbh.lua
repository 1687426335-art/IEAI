...
task.spawn(function()
    ...
    box.Size = UDim2.fromOffset(420, 240)
    box.BackgroundColor3 = Color3.fromRGB(25, 25, 35)
    box.BorderSizePixel = 0
    Instance.new("UICorner", box).CornerRadius = UDim.new(0, 14)
    local st = Instance.new("UIStroke", box)
    st.Color = Color3.fromRGB(0, 255, 170); st.Thickness = 2; st.Transparency = 0.3

    local title = Instance.new("TextLabel", box)
    title.Size = UDim2.new(1, 0, 0, 40); title.Position = UDim2.new(0, 0, 0, 20)
    title.BackgroundTransparency = 1
    title.Text = "请选择你的操作方式"
    title.TextColor3 = Color3.fromRGB(0, 255, 170)
    title.Font = Enum.Font.GothamBold; title.TextSize = 22

    local tip = Instance.new("TextLabel", box)
    tip.Size = UDim2.new(1, 0, 0, 20); tip.Position = UDim2.new(0, 0, 0, 60)
    tip.BackgroundTransparency = 1
    tip.Text = "电脑: 键鼠操作   |   手机: 触屏操作"
    tip.TextColor3 = Color3.fromRGB(200, 200, 200)
    tip.Font = Enum.Font.Gotham; tip.TextSize = 14

    local pcBtn = Instance.new("TextButton", box)
    pcBtn.Size = UDim2.fromOffset(160, 100)
    pcBtn.Position = UDim2.new(0, 30, 0, 100)
    pcBtn.BackgroundColor3 = Color3.fromRGB(60, 100, 180)
    pcBtn.Text = "电脑"; pcBtn.TextColor3 = Color3.new(1,1,1)
    pcBtn.Font = Enum.Font.GothamBold; pcBtn.TextSize = 26
    Instance.new("UICorner", pcBtn).CornerRadius = UDim.new(0, 10)

    local mobileBtn = Instance.new("TextButton", box)
    mobileBtn.Size = UDim2.fromOffset(160, 100)
    mobileBtn.Position = UDim2.new(1, -190, 0, 100)
    mobileBtn.BackgroundColor3 = Color3.fromRGB(60, 170, 100)
    mobileBtn.Text = "手机"; mobileBtn.TextColor3 = Color3.new(1,1,1)
    mobileBtn.Font = Enum.Font.GothamBold; mobileBtn.TextSize = 26
    Instance.new("UICorner", mobileBtn).CornerRadius = UDim.new(0, 10)

    local picked = false
    pcBtn.MouseButton1Click:Connect(function()
        if not picked then picked = true; userMode = "pc"; gui:Destroy() end
    end)
    mobileBtn.MouseButton1Click:Connect(function()
        if not picked then picked = true; userMode = "mobile"; gui:Destroy() end
    end)
end)
...
task.spawn(function()
    ...
    task.spawn(function()
        ...
        if vrPlayers then rig = vrPlayers:WaitForChild(uid, 120) end
    end)
        ...
        while true do
            ...
            pcall(function()
                ...
                local function handle(part, isLeft)
                    ...
                    -- 掉进虚空（Y 低于摄像机 40）
                    if p.Y < (cam.CFrame.Position.Y - 40) then bad = true end
                    -- 离摄像机 120 以上
                    if (p - cam.CFrame.Position).Magnitude > 120 then bad = true end
                    -- 被锚定
                    ...
                    if lastPos and (p - lastPos).Magnitude < 0.001 then
                        stuck = stuck + 0.1
                        if stuck > 1.0 then bad = true end
                    else
                        stuck = 0
                    end
                    ...
                    if bad then
                        ...
                        if isLeft then lastPosL = part.Position; stuckTimeL = 0
                        else lastPosR = part.Position; stuckTimeR = 0 end
                    end
                end
                handle(lh, true); handle(rh, false)
            end)
        end
    end)
    ...
    do
        ...
        -- 如果相机位置离谱（|Y| > 10000 或 NaN），给一个安全值
        if camPos.Y ~= camPos.Y or math.abs(camPos.Y) > 10000 then
            camPos = Vector3.new(0, 100, 0)
        end
    end
    ...
    lp.CharacterAdded:Connect(function(char)
        ...
        if head then camPos = head.Position end
    end)
    ...
    RunService:BindToRenderStep("NoVR_Control", Enum.RenderPriority.Camera.Value + 1, function(dt)
        ...
        -- 确保相机位置没有 NaN
        if camPos.X ~= camPos.X or camPos.Y ~= camPos.Y or camPos.Z ~= camPos.Z then
            local char = lp.Character
            local head = char and char:FindFirstChild("Head")
            camPos = head and head.Position or Vector3.new(0, 100, 0)
        end
    ...
    pcall(function()
        ...
        mf.Size = (userMode == "pc") and UDim2.new(0,360,0,140) or UDim2.new(0,300,0,72)
        ...
        st.Color = Color3.fromRGB(0,255,170); st.Thickness = 1.5; st.Transparency = 0.4

        if userMode == "pc" then
            local title = Instance.new("TextLabel", mf)
            title.Size = UDim2.new(1,-10,0,22); title.Position = UDim2.new(0,5,0,4)
            title.BackgroundTransparency = 1
            title.Text = "VR 手势 :: 电脑模式"
            title.TextColor3 = Color3.fromRGB(0,255,170); title.Font = Enum.Font.GothamBold
            title.TextSize = 14; title.TextXAlignment = Enum.TextXAlignment.Left
            local info = Instance.new("TextLabel", mf)
            info.Size = UDim2.new(1,-10,1,-30); info.Position = UDim2.new(0,5,0,28)
            info.BackgroundTransparency = 1
            info.TextColor3 = Color3.fromRGB(220,220,220)
            info.Font = Enum.Font.Code; info.TextSize = 11
            info.TextXAlignment = Enum.TextXAlignment.Left
            info.TextYAlignment = Enum.TextYAlignment.Top
            info.Text = "鼠标-视角 | WASD-飞行 | 空格/Shift-上/下\n左键/右键-抓取(右/左) | E/Q-捏玩家(右/左)\n滚轮-手距离 | +/-体型 | 中键-旋转手\nF-只右手 | G-只左手 | 数字1~0-手势"
            return
        end

        local t = Instance.new("TextLabel", mf)
        t.Size = UDim2.new(1,-10,0,20); t.Position = UDim2.new(0,5,0,3)
        t.BackgroundTransparency = 1; t.Text = "VR 手势 (手机模式)"
        ...
        local stt = Instance.new("TextLabel", mf)
        stt.Size = UDim2.new(1,-10,0,20); stt.Position = UDim2.new(0,5,0,44)
        stt.BackgroundTransparency = 1; stt.TextColor3 = Color3.fromRGB(255,220,120)
        stt.TextXAlignment = Enum.TextXAlignment.Left; stt.Font = Enum.Font.Code; stt.TextSize = 12
        RunService.Heartbeat:Connect(function()
            h.Text = string.format("动作: %s | 体型: %d/10", Gesture.presetName, S.scale)
            stt.Text = string.format("手距: %.2f", S.reach)
        end)
    end)

    -- 手机模式才创建触屏 UI
    if userMode == "mobile" then
